...
import generateFakeData from '../mock/TropPercuMock'

const TropPercu = ({ fakeData }: { fakeData: DataGridRowTypePercu[] }) => {
  console.log('Received fakeData in TropPercu:', fakeData)
  ...
  useEffect(() => {
    ...
    if (fakeData) {
      const filteredData = fakeData.map((row: DataGridRowTypePercu, index: number) => ({
        id: index + 1,
        Contrat: row.Contrat,
...
export async function getStaticProps() {
  ...
  console.log('Fetched fake data:', fakeData) // Add this line to log the fetched fake data
